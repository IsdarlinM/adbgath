...
from ..modules.wireless import parse_server_status
...
class CapabilityDetector:
    ...
    def detect(self, serial: str) -> dict[str, Any]:
        ...
        server = self.adb.run(["server-status"], timeout=10, check=False)
        server_status = parse_server_status(server.stdout)
        host_tools = {
            name: shutil.which(name)
            for name in ["frida", "frida-ps", "bundletool", "java", "apkanalyzer", "aapt", "aapt2", "apksigner"]
        }
        mdns_enabled = server_status.get("mdns_enabled") is True
        mdns_backend = str(server_status.get("mdns_backend", "")).upper()
        host_mdns_ready = mdns.ok and mdns_enabled
        return {
            ...
            "host": {
                ...
                "mdns": {
                    ...
                    "backend": mdns_backend or "unknown",
                    "recommended_backend": mdns_backend == "LIBADBMDNS",
            ...
            "features": {
                ...
                "wireless_pairing": {
                    "available": sdk >= 30 and host_mdns_ready,
                    "requirement": "Android 11+ and host mDNS",
                },
                "wireless_discovery": {
                    "available": host_mdns_ready,
                    "requirement": "ADB mDNS enabled",
                },
